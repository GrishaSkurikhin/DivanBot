...
func (s *Slider) callback(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	cmdSelectNum := -1
	if num, err := strconv.Atoi(string(cmd[len(cmd)-1])); err == nil {
		cmd = cmd[:len(cmd)-1]
		cmdSelectNum = num
	}
	...
	case cmdSelect:
		if s.deleteOnSelect[cmdSelectNum] {
		...
		s.onSelect[cmdSelectNum](ctx, b, update.CallbackQuery.Message, s.current)
